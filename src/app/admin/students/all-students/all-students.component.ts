  ...
  implements OnInit
{
  ...
  editCall(row: Students) {
    ...
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
      if (result === 1) {
        const updatedData = this.studentsService.getDialogData();
  
        // Check if the new email is already in use by another record (excluding the current record)
        const isEmailInUse = this.exampleDatabase?.dataChange.value.some(
          (x) => x.email === updatedData.email && x.id !== this.id
        );
  
        if (isEmailInUse) {
          this.showNotification(
            'snackbar-danger',
            'Email is already in use by another record. Update failed.',
            'bottom',
            'center'
          );
          return;
  ...
  exportExcel() {
    ...
    const exportData: Partial<TableElement>[] =
      this.dataSource.filteredData.map((x) => ({
        Username: x.username,
        ValidationToken:x.validation_token,
        ApiKey: x.api_key,
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'validationtoken':
          [propertyA, propertyB] = [a.validation_token, b.validation_token];
          break;
        case 'productid':
